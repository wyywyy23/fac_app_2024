...
\begin{frame}{AI Applications Driving Explosive Growth}
    ...
    \note<3-3>[item]{And because there is no way to fit these models into any single computing unit, there has been tremendous effort from the software engineering side to program around data locality, knowing that long-distance data movement is very expensive.}
    ...
    \note<4-4>[item]{So, if these long-distance communications aren't done in a more efficient way, here's what's gonna happen.}
...
\begin{frame}{Per-Training Energy Consumption}
    ...
    \note<4-4>[item]{So, we now know that large-scale computing systems are really about computation AND communication. But what exactly is the problem with today's data communication technologies?}
...
\begin{frame}{Challenges Moving Data Off-Chip}
    ...
    \note<1-1>[item]{Let's take a look at a major bottleneck in today's large-scale computing systems, which is the communication bottleneck. And many have referred to it as the network or bandwidth bottleneck.}
    ...
    \note<4-4>[item]{But this ultra-efficient data movement can't go very far. As we move to the board level, and look between the GPUs, we see that the bandwidth drops by a factor of 5 or so, because the signals need to travel a bit longer in distance and perhaps through a switch fabric. But it is still quite good, well above 1 terabyte per second in those best-in-class commercial solutions. And the energy consumption is about tens to hundreds of femtojoules per bit.}
