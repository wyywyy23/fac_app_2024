...
\begin{frame}{Bringing Photonics into Computing Sockets}
    ...
    \note<3-3>[item]{So, for those of you that I've met on zoom and went over part of my work,}
...
\begin{frame}{What System-Level Impact?}
    ...
    \note<1-1>[item]{I'd like to first bring your attention to this system-level study that I didn't mention, which is really trying to see what can be potentially achieved if we have this ultra-high bandwidth delivered right to the computing socket.}
    ...
    \note<4-4>[item]{and we were delighted to see that we might potentially get a 5 to 7x speedup in the execution time of these workloads, compared to a baseline system using Nvidia H100 GPUs interconnected with NVLinks. So, with this 5 to 7x speedup, imagine the energy saving we can achieve just by having a shorter execution time. And this is in addition to the energy saving we get from the data movement itself.}
