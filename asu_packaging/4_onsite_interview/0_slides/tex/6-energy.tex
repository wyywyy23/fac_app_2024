...
\begin{frame}{Paths to Improvement}
    ...
    \note<3-3>[item]{And this is just the beginning because, as we start to think about a package like this, where it has an integrated comb source, and perhaps the photonics are directly driven by the compute or memory chip, there is a huge space for collaboration, especially with the packaging capabilities here at ASU to think about, in addition to only electronic-photonic integration, how we can make a system with more chiplets integrated on a even larger substrate to further improve the system capabilities.}
